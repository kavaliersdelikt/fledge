...
$dbName = 'fledge-smoke-' + [guid]::NewGuid().ToString('N').Substring(0,10)
...
try {
    $dbId = & docker run -d --name $dbName -e POSTGRES_DB=fledge_ci -e POSTGRES_USER=fledge_ci -e POSTGRES_PASSWORD=ci_test_password_only -p '127.0.0.1::5432' --health-cmd 'pg_isready -U fledge_ci -d fledge_ci' --health-interval 2s --health-timeout 3s --health-retries 30 postgres:16-alpine
    if ($LASTEXITCODE -ne 0) { throw 'Could not create the disposable PostgreSQL container.' }; $startedDb = $true
    $dbPort = $null
    for ($i=0; $i -lt 60; $i++) { & docker exec $dbName pg_isready -U fledge_ci -d fledge_ci *> $null; if ($LASTEXITCODE -eq 0) { $map = & docker port $dbName 5432/tcp; $dbPort = [int](($map -split ':')[-1]); break }; Start-Sleep -Seconds 1 }
    ...
    $env:DATABASE_URL = "postgres://fledge_ci:ci_test_password_only@127.0.0.1:$dbPort/fledge_ci"
