...
try{
 ...
 const first=await app.inject({method:'GET',url:'/api/updates',headers:{authorization:'smoke-admin'}});assert.equal(first.statusCode,200);const result=first.json();assert.equal(result.repository,'kavaliersdelikt/fledge');assert.equal(result.currentVersion,'0.1.0');assert.equal(result.latestVersion,'0.2.0');assert.equal(result.updateAvailable,true);assert.equal(result.body,'Release notes');assert.equal(result.error,null);
