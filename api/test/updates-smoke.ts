...
globalThis.fetch=async(input,init)=>{
 calls++;
 assert.equal(String(input),'https://api.github.com/repos/kavaliersdelikt/navrylo/releases/latest');
 ...
 return Response.json({tag_name:'v0.2.0',name:'Navrylo 0.2.0',html_url:'https://github.com/kavaliersdelikt/navrylo/releases/tag/v0.2.0',body:'Release notes',published_at:'2026-09-25T12:00:00Z'});
};
try{
 ...
 const first=await app.inject({method:'GET',url:'/api/updates',headers:{authorization:'smoke-admin'}});assert.equal(first.statusCode,200);const result=first.json();assert.equal(result.repository,'kavaliersdelikt/navrylo');assert.equal(result.currentVersion,'0.1.0');assert.equal(result.latestVersion,'0.2.0');assert.equal(result.updateAvailable,true);assert.equal(result.body,'Release notes');assert.equal(result.error,null);
