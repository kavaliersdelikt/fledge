...
export default function LiveConsole({id,minecraft,reachable}:{id:string;minecraft:boolean;reachable:boolean}){
 ...
 const [following,setFollowing]=useState(true);const tail=useRef<HTMLPreElement>(null);
 ...
 useEffect(()=>{
  ...
  const connect=()=>{if(stopped||!reachable){setConnection('disconnected');return}setConnection('connecting');
   ...
   ws.onopen=()=>{attempt=0};ws.onmessage=e=>{try{const f=JSON.parse(e.data) as Frame;if(f.type==='status'){setConnection(f.connected?'connected':'disconnected');if(!f.connected)setSample(null)}else if(f.type==='log'&&typeof f.data==='string'){const raw=atob(f.data),bytes=Uint8Array.from(raw,c=>c.charCodeAt(0));const text=decoder.decode(bytes,{stream:true});setOutput(prev=>(prev+text).slice(-120000));}else if(f.type==='sample'&&typeof f.cpuPercent==='number'&&typeof f.memoryBytes==='number'&&typeof f.memoryLimitBytes==='number'){setSample(f)}}catch{setError('Unable to read live data.')}};
   ws.onclose=()=>{setConnection('disconnected');setSample(null);decoder=new TextDecoder();if(!stopped){retry=setTimeout(connect,Math.min(15000,1000*2**Math.min(attempt++,4)))}};ws.onerror=()=>ws?.close();
 ...
 async function send(e:FormEvent){e.preventDefault();const value=command.trim();if(!value)return;setPending(true);setError('');setCommandResult('');try{const result=await json('POST',`/servers/${id}/console`,{command:value}) as {output?:string};setOutput(prev=>(prev+`\n> ${value}\n`).slice(-120000));setCommandResult(result.output||'Input sent to the server.');setCommand('')}catch(ex){setError((ex as Error).message)}finally{setPending(false)}}
 ...
 <div className="console-stream"><div className="console-toolbar"><span className={`signal ${connection!=='connected'?'off':''}`} aria-hidden="true"><i/><i/><i/></span><span>{connection==='connected'?'LIVE STREAM':'AWAITING CONNECTION'}</span><button type="button" className="btn" aria-pressed={following} onClick={()=>setFollowing(!following)}>{following?'Pause scrolling':'Follow output'}</button></div><pre ref={tail} className="console" aria-label="Live server log" style={{maxHeight:480,overflow:'auto',whiteSpace:'pre-wrap',overflowWrap:'anywhere'}}>{output||'Waiting for log data…'}{connection==='connected'&&<span className="console-caret" aria-hidden="true"/>}</pre>
  {commandResult&&<div className="console-ack" role="status"><span>{minecraft?'RCON':'STDIN'}</span>{commandResult}</div>}
  <form className="console-input" onSubmit={send}><label className="sr-only" htmlFor={`console-command-${id}`}>{minecraft?'Minecraft command':'Server console input'}</label><span className="console-prompt" aria-hidden="true">›</span><input id={`console-command-${id}`} value={command} onChange={e=>setCommand(e.target.value)} required maxLength={1024} placeholder={minecraft?'say Hello':'Enter a command or console input…'} disabled={!reachable||connection!=='connected'||pending}/><button className="btn primary" type="submit" disabled={!reachable||connection!=='connected'||pending||!command.trim()} aria-label="Send console input">{pending?'Sending…':<><Send size={15}/>Send</>}</button></form>
  {error&&<div role="alert" className="notice error console-error">{error}</div>}
  {!minecraft&&<p className="console-caption">Sends one line to the container’s standard input. The server software must support console input this way.</p>}
 </div></section>
}
