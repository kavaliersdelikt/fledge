...
try {
  ...
  if ((Get-Location).Path -ne $PSScriptRoot -or (& git rev-parse --verify HEAD 2>$null).Trim() -ne $previous) { try { Invoke-Git @('checkout','--detach',$previous); Invoke-Compose @('up','-d','--build') } catch { Write-Warning 'Automatic code rollback failed; inspect Docker Compose and restore from the database backup if required.' } }
  throw
}
